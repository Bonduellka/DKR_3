...
procedure w;
begin
  print('Приблизительное значение - 1');
  print('Точное значение - 2');
  print('Абсолютная погрешность - 3 ');
  print('Относительная погрешность - 4');
  print('Очистить экран - 0');
end;
...
begin
  repeat
    print('Введите левую границу интервала от (-1.2;0) a=');
    readln(a);
  until (a >= -1.2) and (a <= 0);
  repeat
    print('Введите правую границу интервала (0;2) b=');
    readln(b);
  until (b >= 0) and (b > a) and (b <= 2);
  repeat
    print('Введите число промежутков (5;100) n=');
    readln(n);
  until (n >= 5) and (n <= 100);
  s := (f(a) + f(b)) / 2;  
  for i := 1 to n - 1 do
